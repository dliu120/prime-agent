...
export class AgentSession {
	...
	private async _syncRuntimeStateAfterCompaction(): Promise<void> {
		await this._syncPersistentRuntimeState({
			provisioner: this._ipythonKernelProvisioner,
			isRunning: () => this._ipythonKernelProvisioner?.hasRunningKernel ?? false,
			customType: "ipython_state",
			tag: "ipython_state",
			description: "IPython kernel",
		});
		await this._syncPersistentRuntimeState({
			provisioner: this._bunRuntimeProvisioner,
			isRunning: () => this._bunRuntimeProvisioner?.hasRunningRuntime ?? false,
			customType: "bun_state",
			tag: "bun_state",
			description: "Bun runtime",
		});
	}

	private async _syncPersistentRuntimeState(options: {
		provisioner?: {
			pruneOversizedVariables(signal?: AbortSignal): Promise<string[] | null>;
		...
		if (!provisioner || !options.isRunning()) return;
		const abort = new AbortController();
		const timer = setTimeout(() => abort.abort(), KERNEL_STATE_LISTING_TIMEOUT_MS);
		if (typeof timer === "object" && "unref" in timer) timer.unref();
		let pruned: string[] | null;
		let names: string[] | null;
		try {
			pruned = await provisioner.pruneOversizedVariables(abort.signal).catch(() => null);
			names = abort.signal.aborted ? null : await provisioner.listNamespaceNames(abort.signal).catch(() => null);
		} finally {
			clearTimeout(timer);
		}
		if (names === null && !options.isRunning()) return;
		const detail =
			names === null
				? ""
				: names.length > 0
		...
		const content = [
			`<${options.tag}>`,
			`Your ${options.description} persisted through compaction; its remaining variables, imports, and helpers are still available.${prunedDetail}${detail}`,
			`</${options.tag}>`,
		].join("\n");
		const message = {
			role: "custom" as const,
	...
	private async _performCompaction(options: {
		...
		await this._syncRuntimeStateAfterCompaction();
